...
interface TokenFromLinkProps {
  clientKey : string
}

interface TokenFromLinkState {
  newPassword?:string,
  count?:number,
...
export default class TokenFromLink extends React.Component<TokenFromLinkProps,TokenFromLinkState> {
  
  static contextType = UserContext;

  static async getInitialProps ({query}) {
    //console.log({query});
    return { clientKey : query.token }; 
  }
  ...
  componentDidMount = async () => {
    this.clientKey = decodeURIComponent(document.location.search.split("token=")[1]);
    /**let check the client key from the server response*/
